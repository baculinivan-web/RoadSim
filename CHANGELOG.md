...
- Desktop shell получил минимальное command-driven редактирование demo-проекта:
  изменение ширины полосы, добавление/удаление параллельной дороги и undo/redo
  через `roadsim-commands`. Каждый жест — одна undoable-команда; после успешной
  команды проект перекомпилируется в CSN и подменяет artifact симуляции только
